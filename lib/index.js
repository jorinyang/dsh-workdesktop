...
async function collectFeishuMinutes() {
  ...
  const script = "$ErrorActionPreference='SilentlyContinue'; try { [Console]::OutputEncoding=[Text.Encoding]::UTF8 } catch {}; "
    + '$o = [ordered]@{}; '
    + `$o.owner = (& ${LARK_CLI} minutes +search --owner-ids me --page-size ${FEISHU_MINUTES_PAGE} --as user --format json 2>&1 | Out-String); `
    + `$o.participant = (& ${LARK_CLI} minutes +search --participant-ids me --page-size ${FEISHU_MINUTES_PAGE} --as user --format json 2>&1 | Out-String); `
  ...
  if (env === null) {
    return {
      ok: false, authorized: null, error: 'lark-cli 无输出或非 JSON',
  ...
  if (!okA && !okB) {
    ...
    return {
      ok: false, authorized: e.authorized, error: e.error,
...
function applyScheduleCancels(events, cancels, now = new Date()) {
  ...
  const cancelled = list.filter((c) => inWindow.has(String(c.key)))
  return {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (path === '/schedule/act' && req.method === 'POST') {
        ...
        // 写的是**面板窗口的口径** ⇒ 快照缓存必须失效，否则界面看起来"点了没反应"
        pwbCache.data = null
        pwbCache.at = 0
