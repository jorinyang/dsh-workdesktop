    ...
    ctx.effect(() => tools.register({
      ...
      parameters: {
        ...
        properties: {
          ...
          id: { type: 'string', description: '专注目标 id（declare 时必填）：关注域 id（如 example-client）/ 项目路径 / 事务 id' },
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (path === '/focus' && req.method === 'GET') {
        const r = await run(NODE, [PWB_FOCUS_CLI, 'status'], 30000)
        sendJson(res, 200, parseJsonLoose(r.stdout) ?? { ok: false, error: String(r.error || r.stderr || 'no output').slice(0, 300) })
        return
      }
