    ...
    function apply(ctx) {
      ...
      function Workbench(props) {
        ...
        if (pwbErr !== null) {
          ...
          // ── 多源校验四个数（2026-09-16 接上 `/crosscheck` 只读出口）──────────
          ...
          if (mm && mm.summary) {
            ...
            for (const it of (mm.metrics || [])) {
              const k = rowsBy[it.verdict] ? it.verdict : '无法验证'
              rowsBy[k].push(it.id)
            }
            cardsByKey.set('metrics', card('验收指标 S1–S15',
              '达成 ' + mm.summary.achieved + ' · 未达成 ' + mm.summary.failed,
              [h('div', { className: 'dshw-detail', key: 's' },
                h('div', null, '达成: ' + (rowsBy['达成'].join(' ') || '—')),
                h('div', null, '部分: ' + (rowsBy['部分达成'].join(' ') || '—')),
                h('div', { style: { color: (rowsBy['未达成'].length > 0 ? '#e07070' : undefined) } },
                  '未达成: ' + (rowsBy['未达成'].join(' ') || '—')),
                h('div', { style: { color: '#e0a030' } }, '无法验证: ' + (rowsBy['无法验证'].join(' ') || '—')),
                h('div', null, 'R8 时限: ' + String((mm.r8 || {}).verdict || '—')),
                h('div', null, 'S1 机器侧定位: 最大 ' + String((mm.s1Timing || {}).maxMs || '—') + ' ms'))].concat(xcRows)))
          }
        }
        ...
        if (snap === null) {
          ...
          //   各组织的可以不用细分」）⇒ 去掉按组织的 `组织甲 4/15 · 组织乙 0/2 · …` 徽标。
        ...
        const CARD_ORDER = [
          ...
          'matters', 'triggers', 'schedule', 'todos', 'focus', 'domains',
          'inflow', 'recall', 'objects', 'metrics', 'kb', 'minutes', 'system',
        ]
