...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * ══════════════════════════════════════════════════════════════════════════
     * 【卡名契约·机器可读】卡 key → 显示名（15 项）· **单一真相源**
     * ══════════════════════════════════════════════════════════════════════════
     *
     ...
     *     （动态标题带后缀，如「知识库 · 全部」）；`order` 必须与 host 的 `KNOWN_CARD_KEYS`
     *     与 `CARD_ORDER`、SBT-23 的 WANT 四处逐项一致。
     *
     * [card-titles:begin]
     * {"order":["brief","triggers","matters","schedule","todos","focus","objects","inflow","insights","recall","domains","metrics","kb","minutes","system"],
     *  "titles":{"brief":"决策","triggers":"响应","matters":"事务","schedule":"日程","todos":"待办","focus":"专注","objects":"对象","inflow":"流入","insights":"跨源洞察","recall":"资产","domains":"活跃","metrics":"验收","kb":"知识库","minutes":"近期听记","system":"系统"}}
     * [card-titles:end]
     *
     ...
     *
     * ── 两处**故意保留现状名**、不得"顺手统一" ────────────────────────────────
     *   · `domains` =「活跃」：真机套件 SBT-23/45/46 按**前缀**找这张卡（`startsWith('活跃')`），
     *     改成「活跃关注域」三条同时红；
    ...
    const CARD_TITLES = {
      ...
      domains: '活跃',            // ⚠️ 见上：套件按前缀找「活跃」，暂留旧称
    ...
    const CARD_TITLE_ORDER = ['brief', 'triggers', 'matters', 'schedule', 'todos', 'focus', 'objects', 'inflow', 'insights', 'recall', 'domains', 'metrics', 'kb', 'minutes', 'system']
     ...
     * `TITLE_TO_KEY`：显示名 → key 的**前缀**反查（`kb` 的注册字面量带后缀 `'知识库 · '`，
     *   所以必须前缀匹配而不是全等）。`card()` 靠它拿到本卡的 key，从而**不必改 15 处调用点**。
     */
    const CARD_SCENES = (() => {
      const m = {}
    ...
    /** 由显示名反查 key：全等 → 前缀（`'知识库 · '` 归 `kb`）；查不到 ⇒ null（不猜） */
     ...
     *
     * 为什么必须有：本盘的 15 张卡由**同一个 React 组件**（`Workbench`）渲染。React 的行为是
     ...
     *   其余 14 张照常渲染，整栏（含顺序、拖动、专注降权）不受影响。
     *
    ...
    const CSS = [
      ...
      '.dshw-btab[data-on="1"]{color:var(--dsw-alias-bg-base);background:var(--dsw-alias-brand-primary);border-color:transparent}',
        '.dshw-btab i{font-style:normal;opacity:.75;margin-left:4px;font-size:9.5px}',
     ...
     *
     * [focus-dim:begin]
     * {"nonProminent":{"dim":"data-dim=1（.dshw-focusdim 包装）","folded":"data-folded=1（默认）","unfolded":"data-folded=0（用户手动展开后仍降权）"},"prominentUnchanged":true,"noDisplayNone":true,"domOrderUnchanged":true,"foldSummary":".dshw-focussum（卡名 + 徽标 + 展开/收起按钮；整条不挂 data-click，长按仍可拖动）","targets":{"domain":["domains","objects"],"project":["objects"],"matter":["matters"],"whitelist":["matters","triggers","schedule"],"self":"focus"}}
     * [focus-dim:end]
     */
    const FOCUS_DIM = {
    ...
    function apply(ctx) {
      ...
      function Workbench(props) {
        ...
        //   （不是"点了没反应"），文案里点名缺哪条路由。
        const [adForm, setAdForm] = React.useState({ open: false, id: '', label: '', note: '' })
        ...
        // 客户行下的场次子行**默认收起**（键 = 客户对象 key）；展开状态只在本次会话内
        const [objOpen, setObjOpen] = React.useState({})
        // 「对象」卡的**卡内三个 tab**（2026-09-17 用户追问「我还是不能理解域级和口径在这里的意义是什么，
        //   客户/域级/口径是不是能列成三个tab」）：客户（默认）· 域级/未归属 · 口径。
        //   折叠组的语义是"同一层的分组"，而这三者是**三层不同的东西**（对象层 / 锚不上的记录 / 诊断层），
        //   故改用页签 —— 复用本文件既有的 tab 件（知识库卡 / 响应卡同一套），不新造样式。
        const [objTab, setObjTab] = React.useState('client')
  ...
  return api('/ui-prefs', {
    ...
    .then((r) => { setBrMarkBusy(false); return r })
        ...
        function card(title, badge, body, extra, tall, headInline, opts) {
          ...
          //   这样 15 处 `cardsByKey.set(...)` 调用点**不用逐一改造**，也不会各自写死 key。
          ...
          return h('section', {
            ...
            h('div', { className: 'dshw-card-head' + (headInline ? ' dshw-card-head--nowrap' : '') },
              ...
              //   SPEC §一 要求 15 张卡都有这一格 —— `brief`/`system` 原先传 `null` 故缺失（SBT-90 抓到）。
         ...
         *   **纯字符串前缀剥离**：只认 payload kernel 的三条对象键前缀（客户 / 场次 / 域）——
         *   `客户:示例客户`→`示例客户`、`场次:示例客户-0101`→`示例客户-0101`、`域:example-topic`→`example-topic`；
         *   没有前缀的键（如 `(未归属)`）原样返回。
        ...
        if (pwbErr !== null) {
          ...
          cardsByKey.set('inflow', card('流入',
            ...
            [
              // 降级层：`/disposition` 未加载（旧 host）⇒ 只提示"需重启"，不显示原始路由错误，
              // 下面照旧渲染 `/state` 的 counts（旧口径仍可用）
              routeDown.disposition === true ? downNote('处置台账', 'inflowdown') : null,
              ...
              pendList.length > 0
                ...
                : null,
              // ── ③ 已落点 / ④ 域外拦截（2026-09-16 · 本卡补全四档；二级详情走 portal，下钻不跳页）──
              //  ★ 数据全取自既有 payload：③ = `disposition.entries` 里 landing.type !== 'none'；
              //    ④ = `snapshot.counts.outOfScope`。**本卡不做判断**：原样展示，排序规则可读（entries 已按 source+id 排）。
              //  ★ 为什么写成自包含 IIFE：这一块与上方 ①② 档**互不依赖**，对方冻结后可直接替换/回退，
              //    不必改动它已完成的待议队列与无落点清单。
              (function () {
                const openDetail = (title, kv) => {
                  ...
                  box.style.cssText = 'background:var(--background-primary,#fff);color:var(--text-normal,#222);max-width:640px;max-height:70vh;overflow:auto;padding:14px 16px;border-radius:10px;font-size:13px;line-height:1.6'
                  const hd = document.createElement('div')
                  hd.textContent = title
                  hd.style.cssText = 'font-weight:600;margin-bottom:8px'
                  box.appendChild(hd)
                  for (const kvp of kv) {
                    const v = kvp[1]
                    if (v === null || v === undefined || v === '') continue
                    const r = document.createElement('div')
                    const ks = document.createElement('span'); ks.textContent = kvp[0] + '：'; ks.style.opacity = '.65'
                    const vs = document.createElement('span'); vs.textContent = String(v)
                    r.appendChild(ks); r.appendChild(vs); box.appendChild(r)
                  }
                  const cl = document.createElement('button')
                  cl.className = 'dshw-btn'; cl.textContent = '关闭'
                  cl.onclick = () => document.body.removeChild(wrap)
                  wrap.onclick = (ev) => { if (ev.target === wrap) document.body.removeChild(wrap) }
                  box.appendChild(cl); wrap.appendChild(box); document.body.appendChild(wrap)
                }
                const entries = (dd !== null && Array.isArray(dd.entries)) ? dd.entries : []
                const landed = entries.filter((e) => e && e.landing && e.landing.type !== 'none')
                const landedTop = landed.slice(0, 3).map((e, i) => h('div', {
                  key: 'ld' + i, style: { marginTop: '6px', cursor: 'pointer' },
                  onClick: () => openDetail(String(e.digest_head || e.inbound_id || ''), [
                    ['流入 id', e.inbound_id], ['来源通道', e.source], ['来源路径', e.origin],
                    ['判定', e.verdict], ['判定理由', e.reason], ['判定时间', e.judged_at],
                    ['落点类型', e.landing.type],
                    ['落点 id', (e.landing.matter_ids || []).concat(e.landing.trigger_ids || []).join(', ')],
                    ['落点状态', (e.landing.states || []).join(', ')], ['关联方式', e.landing.match_by],
                  ]),
                }, [
                  h('div', { key: 'a' }, String(e.digest_head || '').slice(0, 60)),
                  h('div', { key: 'b', style: { opacity: 0.7 } },
                    '已落点：' + (e.landing.type === 'both' ? '事务+触发' : e.landing.type === 'matter' ? '事务' : '触发')
                    + ' · ' + ((e.landing.matter_ids || []).concat(e.landing.trigger_ids || []).join(', ') || '—')),
                ]))
                const kids = []
                if (landedTop.length > 0) {
                  kids.push(h('div', { className: 'dshw-detail', key: 'dshw-landed' },
                    [h('div', { key: 'lt' }, '已落点（' + landed.length + ' 条 · 显示最近 3 条 · 点开看落点与出处）')].concat(landedTop)))
                } else if (dd !== null) {
                  kids.push(h('div', { className: 'dshw-detail', key: 'dshw-landed-zero' },
                    '已落点：0 条（口径 = 判定后已派生事务或触发）'))
                }
                if ((c.outOfScope || 0) > 0) {
                  kids.push(h('div', { className: 'dshw-detail', key: 'dshw-out' },
                    '域外拦截（' + c.outOfScope + '）· 我划过的边界，回看用'))
                }
                return kids.length > 0 ? h('div', { key: 'dshw-tail' }, kids) : null
              })(),
            ],
          ...
          if (trigErr !== null) {
            ...
            //  复用知识库卡那套件（容器 `dshw-tabs dshw-tabs-inline` + 页签 `dshw-tab` + `data-on`），
          ...
          //  ＋ 2026-09-17 用户裁定：**加「域 → 对象」下钻**（数据已备，`by_object` 此前为 0 处使用）──
          //  ★ 只读纪律（P2）：一级「域」= `/state` 的 `activeDomains.effective`（自动匹配 / agent 声明
          //    各带自己的 source 与 reason）；二级「对象」= objects.json 里 `domain_id` 命中的对象，
          //    `state` / `counts` / `next_step` **原样取自 payload** —— 本卡不判定、不重排、不补期限、
          //    不把阈值变红绿；**不新增任何写控件**（`重新匹配` 是既有的唯一写入口，未改动）。
          //  ★ 三态分开说（读失败 / 无数据 / 口径为 0 必须文案不同）：
          //      ① 读失败（fetch/解析抛错）→「域 → 对象 读取失败: …」；
          //      ② 出口不可用（503 / 响应里没有 objects）→「域 → 对象 不可用: …（属于读失败，不等于 0 个对象）」；
          //      ③ 无数据（`objects = []`）→「无数据：… objects = 0 个」；
          //      ④ **口径为 0**（对象层有数据、但这个域名下 0 个对象）→「该域下 0 个对象（口径结果，
          //         不是读失败）」—— 例如只声明了域、还没有任何落点。
          //  ★ 展开态存 localStorage（只影响渲染，不写 host、不进任何契约）。
          const ad = pwb.activeDomains
          const adEff = (ad && ad.effective) || []
          const adOpen = lsGet('dshw.domOpen') !== '0'   // 默认**展开**：下钻是要一眼看见的能力
          const adObjIdx = objDomainIndex(objs)
          const adRows = adEff.length === 0
            // ★ 三态分开说（框架层 2026-09-17 裁定）：**读失败 / 未声明未匹配 / 声明了但口径为 0** 三者文案互异
            ? [empty(ad === null || ad === undefined
              ? '关注域不可用：/state 未返回 activeDomains（属读失败，不等于 0 个域）'
              : (ad.source === 'declared'
                ? '已声明但生效列表为空（口径结果：声明未生效或被清除）'
                : '未声明也未匹配（点「重新匹配」读当前会话；或点「声明」手动指定）'), 'noad')]
            : adEff.map((d, i) => domainObjRow(d, i, adOpen, adObjIdx))
          adRows.push(h('div', {
            className: 'dshw-orderlink', key: 'adfold', 'data-click': '1',
            title: adOpen ? '把每个域下的对象列表收起来（一级只看域）' : '展开每个域下的对象（域 → 对象 下钻）',
            onClick: () => {
              const nxt = adOpen !== true
              lsSet('dshw.domOpen', nxt ? '1' : '0')
              setDomOpen(nxt)
            },
          }, adOpen ? '收起域下对象' : '展开域下对象'))
          ...
          const adObjTotal = objHasDocShape(objs) === true
            ? objs.objects.filter((o) => o && o.kind !== 'unassigned' && o.domain_id !== null && o.domain_id !== undefined).length
            : null
          ...
          function declareActiveDomainNow() {
            ...
            adWrite('declare', {
              ...
              note: String(adForm.note || '').trim(),
            })
              .then((r) => {
                if (r && r.ok === true) {
                  setAdMsg({ ok: true, text: '已声明 ' + id + '（生效 = 声明覆盖自动匹配）' })
                  setAdForm((p) => Object.assign({}, p, { open: false, id: '', label: '', note: '' }))
                  loadPwb(true)
          ...
          if (adForm.open === true) {
            adRows.push(h('div', { className: 'dshw-adform', key: 'adform', 'data-ad-form': '1' },
              h('input', {
                className: 'dshw-input', 'data-ad-field': 'id', placeholder: '关注域 id（注册表里的 id，如 qianwen-office）',
                value: adForm.id, onChange: (ev) => setAdForm((p) => Object.assign({}, p, { id: ev.target.value })),
              }),
              h('input', {
                className: 'dshw-input', 'data-ad-field': 'label', placeholder: '显示名（可空，默认取 id）',
                value: adForm.label, onChange: (ev) => setAdForm((p) => Object.assign({}, p, { label: ev.target.value })),
              }),
              h('input', {
                className: 'dshw-input', 'data-ad-field': 'note', placeholder: '说明（可空，写进声明的 note）',
                value: adForm.note, onChange: (ev) => setAdForm((p) => Object.assign({}, p, { note: ev.target.value })),
              }),
              h('div', { className: 'dshw-btnrow' },
                h('button', { className: 'dshw-btn', disabled: adBusy === true, onClick: declareActiveDomainNow }, adBusy === true ? '…' : '确认声明'),
                h('button', { className: 'dshw-btn', onClick: () => setAdForm((p) => Object.assign({}, p, { open: false })) }, '取消')),
              h('div', { className: 'dshw-detail' },
                '写入 active-domains.json，与 agent 工具 workbench_active_domains 同一通道（host 不新增判定）；域 id 必须在注册表内，非法 id 由库内 CLI 拒绝并回报原因')))
          }
          if (adMsg !== null) {
            adRows.push(h('div', {
          ...
          cardsByKey.set('domains', card('活跃',
            adEff.length === 0 ? '未匹配' : adEff.length + ' 个' + (adObjTotal === null ? '' : ' · 对象 ' + adObjTotal),
            adRows,
            h('span', { className: 'dshw-btnrow' },
              h('button', {
                className: 'dshw-btn',
                onClick: () => {
              ...
              h('button', {
                ...
                onClick: () => setAdForm((p) => Object.assign({}, p, { open: p.open !== true })),
        ...
        // ⚠️ 数据时刻那一项**不进卡面**（用户 2026-09-17 再更正：那是生产侧自检信息）；它只在
        //    **二级详情 ③**（核对出处的审计语境）里取值 —— 取值就地做在那个弹窗里，本段不取。
        const objRollupKeys = Object.keys(objRollup)
        // ⚠️ `objRaw` / `objCountsText` / `objTrajTag` / `openObj` / `objItemRow` 已**统一上移**到
        //    卡片装配段开头（「对象行 / 二级详情的共用件」）——「活跃」卡的域→对象下钻比本段更早注册，
        //    就地定义会踩 TDZ。这里**不再重复声明**（重复声明即 `SyntaxError`，node --check 可查）。
        ...
        /**
         * 弱尾节点（2026-09-17 用户裁定）：`38 件事 · 17 条承诺` + 仅触发/仅事务 —— 与主文本列**同一行**、
         * 但字号更小、无底色、不可点。它不参与「对象名 · 状态 · 下一步」这三列，也不进主文本。
         * tooltip 保留 payload 的**原字段名**（matters / triggers），措辞说人话但账仍可核。
         */
        const objWeak = (c, traj, k) => h('span', {
          className: 'dshw-objweak', key: k,
          title: 'payload counts（原样，本卡不重算）: ' + objCountsRaw(c) + objTrajTag(traj),
        }, objCountsText(c) + objTrajTag(traj))
        ...
        // ── 卡内两个 tab（2026-09-17 用户追问「域级和口径在这里的意义是什么」→ 再更正：取消口径）──
        //  tab 条挂**卡头**（`card()` 第 4 参 `extra`），复用知识库卡那套件
        //  （容器 `dshw-tabs dshw-tabs-inline` + 页签 `dshw-tab` + `data-on`；CSS 里
        //   `flex:1 1 100%;order:9;justify-content:flex-start` ⇒ 独占一行、左对齐、排在卡头最后）。
        //  两个 tab 的卡体行**各自分桶**装好，最后只把当前 tab 的行铺进卡体 —— 不合并、不丢组、不重排。
        //  ⚠️ 卡面**不出现生产侧自检信息**（用户 2026-09-17 再更正：覆盖率类计数、颗粒度分布、
        //     来源方式、数据时刻、kernel 段、bySide 段一律不上卡 ——「这些数字可不可信」是管线/agent
        //     的门槛，不是看卡的人要读的东西）。
        //     卡上唯一保留的 payload 自检字段是 `coverage.unassigned` 一项，且只作**可行动**的业务话术
        //     （"有 2 条记录没挂到任何客户 · 待你整理归属"）；其余 coverage 字段一概不读、不渲染。
        let objTabBar = null
        const objClientRows = []
        const objNonRollupRows = []
        const objRows = []
        if (objsErr !== null) {
          ...
          // ⚠️ 只取 `coverage.unassigned` 一项（**可行动**的业务信息：有几条记录挂不上任何客户）；
          //    覆盖率的其余字段（记录数 / 已锚定 / 颗粒度 / 派生方式）**不上卡** —— 见上方再更正注释。
          const cov = objDoc.coverage || {}
          if (objInPlay.length === 0) {
            // 第三种空态：产物有对象，但按口径**在场为 0** —— 既不是读失败，也不是没有数据。
            // 只留一句「口径的结果」，不再在卡头报任何聚合数（用户 2026-09-17 复查）。
            // ⚠️ 2026-09-17 再更正：**不再指路到已取消的「口径」组**（生产侧自检信息不上卡）。
            objRows.push(h('div', { className: 'dshw-detail', key: 'objzero', style: { color: '#e0a030' } },
              '在场为 0：按卡片口径（state ≠ settled 且 事务+触发 ≥ 2）在场的对象为 0 个 —— 这是口径的结果，'
              + '不是读失败；可归属的记录在「客户」tab，挂不上客户的在「域级/未归属」tab'))
          }
          // 顶层行 = 客户级（rollup 的键）；场次只在展开时出现 —— 全部进 tab ①「客户」
          const objShownKeys = objShowAll === true ? objRollupKeys : objRollupKeys.slice(0, MAX_ROWS)
          if (objRollupKeys.length === 0) {
            objClientRows.push(empty('客户级 rollup 为空（payload rollup 段 0 个键）', 'objnorollup'))
          }
          for (let i = 0; i < objShownKeys.length; i += 1) {
            ...
            const nx = nxItem === null || nxItem.next_step === null || nxItem.next_step === undefined ? null : nxItem.next_step
            const ruObj = objByKey[k] === undefined ? null : objByKey[k]
            const evText = objEvidenceText(ruObj === null ? null : ruObj.evidence)
            // 机器 id（对象键 / 下一步 id）收进 tooltip：行上先说人话（2026-09-17 用户反馈）
            const rowTip = ['客户级（rollup）· 子对象 ' + kids.length + ' 个（场次折叠在下一层）',
              '对象键: ' + String(ru.key || k), evText]
            if (nx !== null) {
              rowTip.push('下一步 id: ' + String(nx.id) + '（来自对象 ' + String(nxItem.key || '') + '，payload next_step 原样显示）')
            }
            rowTip.push('点击查看二级详情（只读）')
            objClientRows.push(h('div', { key: 'oc-' + k },
              h('div', {
                className: 'dshw-item', 'data-click': '1',
                title: rowTip.join('\n'),
                onClick: () => openObj(k, objByKey[k] || null),
              },
              // ① 对象名（剥掉机器前缀）② 状态（纯文本，不配色）③ 下一步**先说标题** —— 全部 payload 原样
              // 次行 = 依据（payload evidence）：一行读下来知道"这是什么 / 该干什么 / 凭什么"
              objTwoLine(evText, [
                h('span', { key: 'n' }, objLabel(ru.key || k)),
                h('span', { key: 'st' }, ' · ' + String(ru.state || '—')),
                nx === null
                  ? h('span', { className: 'dshw-objnext', key: 'ns' }, ' · 下一步: payload 未给出')
                  : h('span', {
                    className: 'dshw-objnext',
                    key: 'ns',
                    title: '下一步 id ' + String(nx.id) + ' · 来自对象 ' + String(nxItem.key || '') + '（payload next_step 原样显示）· 点击查看二级详情',
                  }, ' · 下一步 ' + String(nx.title || nx.id || '—') + ' · ' + objRaw(nx.due_at)
                    + ' · ' + String(nx.owner || '—')),
              ]),
              // 弱尾（用户 2026-09-17 裁定）：计数与仅触发/仅事务标签退出主列，退成末尾灰字
              objWeak({ matters: ru.matters, triggers: ru.triggers }, String(ru.trajectory || ''), 'w'),
              kids.length > 0
                ? h('button', {
          ...
          //  ⚠️ 这一层只写**业务语言**：这是什么、要不要人管；不写覆盖率/颗粒度这类生产侧自检数。
          const objUnassignedN = cov.unassigned === null || cov.unassigned === undefined ? 0 : Number(cov.unassigned)
          if (objUnassignedN > 0) {
            objNonRollupRows.push(h('div', { className: 'dshw-detail', key: 'objorphact' },
              String(objUnassignedN) + ' 条记录没挂到任何客户 · 待你整理归属'))
          }
          objNonRollupRows.push(h('div', { className: 'dshw-detail', key: 'objorphnote' },
            '这一层是**没有一个客户可归属**的记录：域级对象 = 只挂到关注域；'
            + '未归属 = 三条规则都不成立（不猜）；父对象不在客户级 rollup 内的场次也在这一层。'
            + '点任意一行看二级详情（只读）。'))
          if (objOrphans.length === 0) {
            objNonRollupRows.push(empty('这一层为 0：payload 里没有挂不上客户的记录（不是读失败）', 'objorph0'))
          } else {
            for (let j = 0; j < objOrphans.length; j += 1) objNonRollupRows.push(objItemRow(objOrphans[j], true))
          }
          // ── tab 条（挂卡头）+ 当前 tab 的卡体 ──────────────────────────────
          //  默认 tab = 客户；页签标题**自解释**（用户原话：看不懂域级是什么意思），每条页签带自己的条数；
          //  空组**保留页签并标 0**（隐藏会让人以为这一层丢了）。
          const objTabDefs = [
            {
              key: 'client', label: '客户', n: objRollupKeys.length,
              hint: '客户级对象（payload rollup 的键）；场次子行点行尾箭头展开',
            },
            {
              key: 'nonrollup', label: '域级/未归属（挂不上客户的记录）', n: objOrphans.length,
              hint: '锚不到客户的记录：域级对象 / 未归属 / 父对象不在客户级 rollup 内的场次',
            },
          ]
          objTabBar = h('div', { className: 'dshw-tabs dshw-tabs-inline' },
            objTabDefs.map((g) => h('button', {
              key: 'objtab-' + g.key,
              className: 'dshw-tab',
              'data-on': objTab === g.key ? '1' : null,
              'data-obj-tab': g.key,
              onClick: () => setObjTab(g.key),
              title: g.hint,
            }, g.label + ' (' + String(g.n) + ')')))
          const objActiveRows = objTab === 'nonrollup' ? objNonRollupRows : objClientRows
          for (let i = 0; i < objActiveRows.length; i += 1) objRows.push(objActiveRows[i])
        }
        ...
        cardsByKey.set('objects', card('对象', objBadge, objRows, objTabBar))
        ...
        const brTotalOf = (gk) => {
          ...
          // 昨夜动向的 tab 徽标取 `overnight.total`（产物字段；未回退时为窗口内全部条数，
          //   回退中为该 tab 列出的条数）——**不去数被截断的 items**（用户 2026-09-17 裁定）。
         ...
         */
        const brRowOf = (seg, it, key) => {
          const rk = brItemKey(it)
          const isRead = it !== null && it !== undefined && it.read === true
          return h('div', {
            className: 'dshw-item', key,
            'data-click': '1', 'data-read': isRead ? '1' : '0', 'data-bread-key': rk,
            title: '所属段 ' + seg + '\n标题 ' + String(it.title || it.what || '')
            ...
            h('span', { className: 'dshw-item-time' }, brWhen(it.due_day || it.due_at || it.at)),
            h('span', { className: 'dshw-item-text' },
              String(it.title || it.what || '(无标题)').slice(0, 56)
        ...
        const brRows = (() => {
          ...
          out.push(h('div', { className: 'dshw-btabs', key: 'brtabs' }, brGroups.map((g) => {
            ...
            }, String(gg.label || k), h('i', null, String(brTotalOf(k))))
          })))
          ...
          // 渲染**当前 tab**：三档各自的结构
          {
            ...
            if (gk === 'todayMustDo') {
              ...
              for (let bi = 0; bi < brBucketKeys.length; bi += 1) {
                ...
                for (let ri = 0; ri < bitems.length; ri += 1) {
                  out.push(brRowOf(blabel, bitems[ri], 'brr-' + bk + '-' + ri))
                }
              }
              return out
            }
            ...
            if (gk === 'waiting') {
              ...
              for (let ri = 0; ri < shown.length; ri += 1) {
                ...
                out.push(brRowOf(glabel, it, 'brr-waiting-' + ri))
              }
              return out
            }
            // 昨夜动向：一档一列，超上限时由组头的「展开全部」照列 payload 明细
            const gItems = brItemsOf(gk)
            const shown = gItems
            for (let ri = 0; ri < shown.length; ri += 1) {
              out.push(brRowOf(glabel, shown[ri], 'brr-' + gk + '-' + ri))
            }
            if (gk === 'overnight' && gTotal > gItems.length) {
              out.push(h('div', { className: 'dshw-detail', key: 'brovnote', style: { opacity: 0.7 } },
                '窗口内 ' + String(gTotal) + ' 条；产物只附明细 ' + String(gItems.length)
                + ' 条（tab 徽标取产物 total；其余明细未随附）'))
            }
            return out
          }
        })()
        ...
        const brfModalEl = brfModal === null ? null : (() => {
          ...
          secs.push(h('div', { className: 'dshw-modal-sec', key: 'b1' },
            ...
            [
              brModalRow('标题', String(it.title || it.what || '—')),
        ...
        const insModalEl = insModal === null ? null : (() => {
          ...
          secs.push(h('div', { className: 'dshw-modal-sec', key: 'i2' },
            h('div', { className: 'dshw-modal-sec-h' }, '② 证据链（逐条 · 点击打开出处 · ' + evs.length + ' 项）'), evRows))
        ...
        //    **最终顺序（15 项 · 三带布局 · 用户裁定 2026-09-16）**：
        //      热层：决策（旧称「今日决策面」）→ 待响应触发 → 事务 → 日程 → 待办
        //      在办层：专注 → 对象 → 流入 → 跨源洞察 → 资产
        //      参考层：活跃关注域 → 验收 → 知识库 → 近期听记 → 系统状态
        ...
        const CARD_ORDER = [
          // 用户裁定 2026-09-16 · **三带布局**（热层 / 在办层 / 参考层）· 15 项
          ...
          'focus', 'objects', 'inflow', 'insights', 'recall',                 // 在办层：这件事到哪了
          'domains', 'metrics', 'kb', 'minutes', 'system',                    // 参考层：查阅与诊断
        ]
        ...
        return h('div', {
          ...
          'data-modal': (matterModal !== null && matterModalNode === matterModalEl)
            ...
            || (trigModal !== null && trigModalNode === trigModalEl) ? '1' : null,
        }, panel, matterModalNode, calModalNode, todoModalNode, objModalNode, mmModalNode, trigModalNode, brfModalNode, insModalNode, foModalNode)
      }
